...
class NASNetAMobile(nn.Module):
    ...
    def __init__(self, num_classes=1000, stem_filters=32, penultimate_filters=1056, filters_multiplier=2):
        ...
        self.conv0.add_module('conv', nn.Conv2d(in_channels=8, out_channels=self.stem_filters, kernel_size=3, padding=0, stride=2,
                                                bias=False))
...
if __name__ == "__main__":
    ...
    model = NASNetAMobile()
    test_input = torch.zeros(2, 8, 224, 224)
    test_input[:, :, 106:118, 106:118] = 1
    input = Variable(test_input)
    model_features, output = model(input)

    print(model_features.size())
    print(model_features[0, 0, :, :])
    print(model_features[0, 0, 2:5, 2:5])
    print(output.size())